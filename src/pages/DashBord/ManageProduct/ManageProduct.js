...
const ManageProduct = () => {
  ...
  return (
    <div className='container mt-3 text-center'>
      <h2 className="fw-bold text-uppercase mb-4">Manage All<span className='text-info'> Products</span> </h2>
      {products.length ? <Row xs={1} md={3} className="gy-5 gx-4 text-start container mx-auto mb-4">
        {
          products.map(product =>
            <Col key={product?._id}>
              <Card className="px-2 py-2">
                <Card.Img variant="top" src={product?.img} className="px-2 mx-auto" height="150px" />
